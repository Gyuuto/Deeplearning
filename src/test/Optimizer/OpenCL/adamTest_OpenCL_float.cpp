...
namespace {
    ...
    TEST_F(ADAMTest, OpenCL_float_update_test) {
        std::vector<clMatrix<Real>> update_W, update_b;
        update_W.emplace_back(clMatrix<Real>(nabla_W[0].m, nabla_W[0].n));
        update_b.emplace_back(clMatrix<Real>(nabla_b[0].m, nabla_b[0].n));

        ADAM<clMatrix, Real> adam(nabla_W, nabla_b, EPS);
