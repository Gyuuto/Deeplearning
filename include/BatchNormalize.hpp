...
#endif
	std::vector<std::vector<Mat>> nabla(1, std::vector<Mat>(num_map, Mat(1, 2)));
...
#ifdef USE_MPI
					tmp_ret[i](j, k) = W[0][i](0,0)*(U[i](my_offset+j,k) - mean(i,j))/std::sqrt(var(i,j)+EPS) + W[0][i](0,1);
#else
					ret[i](j, k) = W[0][i](0,0)*(U[i](my_offset+j,k) - mean(i,j))/std::sqrt(var(i,j)+EPS) + W[0][i](0,1);
#endif					
				}
			}
		}
